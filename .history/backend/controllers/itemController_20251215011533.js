...
exports.getAllItems = async (req, res) => {
  try {
    ...
    const items = await Item.find({ status: 'active' })
      .select("title pricePerDay location province images rating status _id category")
      .skip((page - 1) * limit)
      .limit(limit)
      .lean()
      .exec();
...
exports.getItemsByOwner = async (req, res) => {
  try {
    const items = await Item.find({ owner: req.params.ownerId })
      .select("_id title pricePerDay location province images category status views totalBookings createdAt updatedAt availability")
      .lean();
