...
const MyRentalsPage = () => {
  ...
  const [hiddenInAll, setHiddenInAll] = useState(new Set());
  ...
  const handleRemoveCancelled = (bookingId) => {
    setHiddenInAll((prev) => new Set(prev).add(bookingId));
  };

  const filteredRentals = rentals.filter((r) => {
    if (filter === "all") {
      return !hiddenInAll.has(r._id); // hide removed cancelled items
    }
  ...
  const getEmptyTitle = () => {
    if (filter === "all") return "No Rentals Yet";
    return `No ${filter.charAt(0).toUpperCase() + filter.slice(1)} Rentals Yet`;
  };

  const getEmptyDescription = () => {
    switch (filter) {
      case "approved":
        return "You don’t have any approved rentals yet. Stay tuned — owners may approve your bookings soon.";

      case "pending":
        return "You don’t have any pending rentals yet. Browse items and book now to get started!";

      case "completed":
        return "You haven’t completed any rentals yet. Once your rentals finish, they’ll appear here.";

      case "cancelled":
        return "You don’t have any cancelled rentals. That’s a good thing!";

      default:
        return "You haven't rented any items yet. Start exploring and find something amazing to rent!";
    }
  };
      ...
      <div className="flex-1 pl-6 pb-20 pt-10">
        {loading ? (
          ...
          <div className="grid grid-cols-1 gap-4">
            {sortedRentals.map((booking) => (
              <RentalCard
                ...
                onRemove={handleRemoveCancelled}
              />
            ))}
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center h-full pt-16">
            ...
            <p className="text-gray-600 mb-8 text-md max-w-md text-center">
              {getEmptyDescription()}
            </p>
