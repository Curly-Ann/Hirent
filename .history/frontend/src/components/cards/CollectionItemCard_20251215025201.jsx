        ...
        <div className="flex-1 flex flex-col justify-between">
          <div>
            ...
            <div className="text-[13px] mt-1 text-gray-700">
              <div className="flex items-center gap-3 flex-wrap">
                <span>
                  ...
                  <span className="font-medium text-gray-900">
                    {item.itemId?.owner?.name || "Unknown"}
                  </span>
                </span>
              ...
              {/* STATUS BADGE */}
              <div
                className={`absolute top-1 right-0 inline-flex items-center gap-1 text-[13px] font-medium px-2.5 py-1 rounded-full ${
              ...
              >
                ...
                {!item.bookingStatus && <CalendarOff className="w-3 h-3" />}
                ...
                <div className="flex flex-col gap-1">
                  ...
                  {item.status !== "approved" && item.status !== "pending" && (
                    <div className="flex items-center gap-1 text-gray-800 text-[13px] mb-3">
                      <Calendar size={15} />
                      <span>
                        {item.itemId?.minimumRentalDays ?? 1}
                        {" - "}
                        {item.itemId?.maximumRentalDays ?? "∞"} days available
                      </span>
                    </div>
                  )}
          ...
          <div className="absolute bottom-1 right-0 flex items-center gap-1.5">
            ...
            item.bookingStatus === "pending" ? (
              ...
              <button
                onClick={() =>
                  handleRemoveItem(item.itemId?._id || item.itemId)
                }
                className="px-3 py-1.5 text-[12.5px] shadow-sm rounded-full text-red-500 border border-red-300 bg-red-50 hover:bg-red-100"
              >
            ...
            {item.bookingStatus === "approved" ? (
              <button
                onClick={() =>
                  alert(`Contacting owner: ${item.itemId?.owner?.name}`)
                }
              ...
              <button
                onClick={() =>
                  navigate(`/booking/${item.itemId?._id || item.itemId}`)
                }
                className="px-3 py-1.5 text-[12.5px] shadow-sm bg-[#7A1CA9] text-white rounded-full hover:bg-purple-800"
              >
                Continue to Booking
