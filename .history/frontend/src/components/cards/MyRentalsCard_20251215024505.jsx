...
const RentalCard = ({
  ...
  return (
    <div className="relative bg-white rounded-2xl border p-4 shadow-sm hover:shadow-md transition">
      ...
      <div className="flex gap-5">
        ...
        <div className="flex-1 flex flex-col justify-between">
          <div>
            ...
            <div className="flex items-center gap-2 text-[13px] text-gray-600 mt-1">
              <User className="w-4 h-4" /> Listed by{" "}
      ...
      <div className="text-[13px] text-gray-900 mt-3 flex items-center gap-2">
  <Calendar className="w-4 h-4" />
  {dayjs(booking.startDate).format("M/D/YYYY")} → {dayjs(booking.endDate).format("M/D/YYYY")}

  <span className="flex items-center gap-1 text-yellow-600 ml-4">
    <Clock className="w-4 h-4" />
    {dayjs(booking.endDate).diff(dayjs(booking.startDate), "day")}{" "}
    {dayjs(booking.endDate).diff(dayjs(booking.startDate), "day") > 1 ? "days" : "day"}
  </span>
</div>
          ...
          <div className="flex justify-end gap-2 mt-2">
            {booking.status === "cancelled" && currentFilter === "all" ? (
              <button
